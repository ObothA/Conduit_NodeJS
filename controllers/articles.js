...
module.exports.createArticle = async (req, res) => {
  try {
    ...
    if (data.categoryList) {
      for (let c of data.categoryList) {
        const categoryExists = await Category.findByPk(c.name);
        let newCategory;

        if (!categoryExists) {
          newCategory = await Category.create(c);
          await article.addCategory(newCategory);
        } else {
          await article.addCategory(categoryExists);
        }
      }
    }

    article = await Article.findByPk(slug, { include: [Tag, Category] });
...
module.exports.getSingleArticleBySlug = async (req, res) => {
  try {
    ...
    console.log('HEllo');
    console.log(slug);
    let article = await Article.findByPk(slug, { include: [Tag, Category] });
...
module.exports.updateArticle = async (req, res) => {
  try {
    ...
    let article = await Article.findByPk(slugInfo, {
